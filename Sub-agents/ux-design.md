...
**After Design Gate Ship ✓ — write fixture-spec.md (Fast Track):**
In Full Diamond mode, fixture-spec.md is written by define-agent from discovery output.
In Fast Track mode, no define-agent runs — so you write it instead, after Design Gate passes.

Check: does `fixture-spec.md` already exist?
- Yes → append your new scenarios under a new section heading
- No → create it from scratch using this structure:

```
# fixture-spec.md

## Source
Fast Track — generated by ux-design after Design Gate Ship ✓

## Hypothesis scenarios
| ID | Scenario | Hypothesis | Expected outcome | Risk if wrong |
|---|---|---|---|---|
| H-01 | [user action] | [assumption being tested] | [what success looks like] | [consequence of false assumption] |

## Edge cases
- [scenario that could break the design]

## Open questions for validation-agent
- [what still needs to be tested with users or stakeholders]
```

Derive scenarios directly from your UX Direction Brief — one hypothesis per key design decision.
Minimum 3 scenarios. Flag each with the design decision it validates.
